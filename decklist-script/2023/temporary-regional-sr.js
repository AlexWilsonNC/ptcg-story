const masterList = document.querySelector('.seniors-ol');
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (item.list.includes('./assets/list-icon.png')) {
            ...
            listIcon.addEventListener('click', function () {
                ...
                document.querySelector('.playerName').innerHTML = convertToTitleCase(item.firstName) + "<br><i>" + item.event + " - Seniors</i>";
                if (item.disclaimer) {
                ...
                for (const cardType in item.decklist) {
                    item.decklist[cardType].forEach(card => {
                        ...
                        const allSets = {
                            // sv
                            sv6, sv5, sv4pt5, fr, sv4, sv3pt5, sv3, sv2, sv1, svp, sve, importedenergy,
                            //swsh
