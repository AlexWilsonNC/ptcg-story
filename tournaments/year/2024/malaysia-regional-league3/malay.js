"masters": [
    {
        "name": "Hariz Hamdan",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "giratina-origin",
        "sprite2": "comfey",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Lim Ken Hoong ",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "gardevoir",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Yoong Kai Jian",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "mew-vmax",
        "sprite2": "genesect",
"decklist": {
            "pokemon": [
                { "count": 3, "name": "Mew VMAX", "set": "FST", "number": "114" },
                { "count": 4, "name": "Mew V", "set": "FST", "number": "113" },
                { "count": 4, "name": "Genesect V", "set": "FST", "number": "185" },
                { "count": 1, "name": "Meloetta", "set": "FST", "number": "124" },
                { "count": 1, "name": "Eiscue", "set": "BRS", "number": "44" },
            ], "trainer": [
                { "count": 2, "name": "Elesa's Sparkle", "set": "FST", "number": "233" },
                { "count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 1, "name": "Iono", "set": "PAF", "number": "80" },
                { "count": 1, "name": "Judge", "set": "SVI", "number": "176" },
                { "count": 4, "name": "Ultra Ball", "set": "SVI", "number": "196" },
                { "count": 4, "name": "Battle VIP Pass", "set": "FST", "number": "225" },
                { "count": 4, "name": "Power Tablet", "set": "FST", "number": "236" },
                { "count": 3, "name": "Cram-o-matic", "set": "FST", "number": "229" },
                { "count": 3, "name": "Lost Vacuum", "set": "CRZ", "number": "135" },
                { "count": 3, "name": "Forest Seal Stone", "set": "SIT", "number": "156" },
                { "count": 2, "name": "Escape Rope", "set": "BST", "number": "125" },
                { "count": 2, "name": "Box of Disaster", "set": "LOR", "number": "154" },
                { "count": 1, "name": "Switch", "set": "SVI", "number": "194" },
                { "count": 1, "name": "Pal Pad", "set": "SVI", "number": "182" },
                { "count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146" },
                { "count": 1, "name": "Nest Ball", "set": "SVI", "number": "181" },
                { "count": 1, "name": "Choice Belt", "set": "PAL", "number": "176" },
                { "count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177" },
                { "count": 2, "name": "Path to the Peak", "set": "CRE", "number": "148" },
                { "count": 1, "name": "Town Store", "set": "OBF", "number": "196" },
            ], "energy": [
                { "count": 4, "name": "Fusion Strike Energy", "set": "FST", "number": "244" },
                { "count": 3, "name": "Double Turbo Energy", "set": "BRS", "number": "151" },
            ]
        }
    },
    {
        "name": "Magic",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "chien-pao",
        "sprite2": "baxcalibur",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "YinWei",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "charizard-shiny",
        "sprite2": "comfey",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "MSPang",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "gardevoir",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Syahmi Razak",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "roaring-moon",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Ganza11",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "giratina-origin",
        "sprite2": "comfey",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Sam",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "roaring-moon",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "hlwong",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "gholdengo",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Low Kien Shieng",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "roaring-moon",
"decklist": {
            "pokemon": [
                { "count": 4, "name": "Roaring Moon ex", "set": "PAR", "number": "124" },
                { "count": 2, "name": "Galarian Moltres V", "set": "CRE", "number": "97" },
                { "count": 1, "name": "Mew ex", "set": "MEW", "number": "151" },
                { "count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46" },
                { "count": 1, "name": "Squawkabilly ex", "set": "PAF", "number": "75" },
                {"count": 1, "name": "Morpeko", "set": "PAR", "number": "121"},
            ], "trainer": [
                { "count": 3, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170" },
                { "count": 2, "name": "Professor's Research", "set": "PAF", "number": "87" },
                { "count": 1, "name": "Iono", "set": "PAF", "number": "80" },
                { "count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 4, "name": "Battle VIP Pass", "set": "FST", "number": "225" },
                { "count": 4, "name": "Dark Patch", "set": "ASR", "number": "139" },
                { "count": 4, "name": "Energy Switch", "set": "SVI", "number": "173" },
                { "count": 3, "name": "Ultra Ball", "set": "SVI", "number": "196" },
                { "count": 3, "name": "Escape Rope", "set": "BST", "number": "125" },
                { "count": 3, "name": "Pokémon Catcher", "set": "SVI", "number": "187" },
                { "count": 2, "name": "Earthen Vessel", "set": "PAR", "number": "163" },
                { "count": 2, "name": "Trekking Shoes", "set": "CRZ", "number": "145" },
                { "count": 1, "name": "Pal Pad", "set": "SVI", "number": "182" },
                { "count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156" },
                { "count": 1, "name": "Emergency Jelly", "set": "SIT", "number": "155" },
                { "count": 1, "name": "Ancient Booster Energy Capsule", "set": "TEF", "number": "140" },
                { "count": 3, "name": "PokéStop", "set": "PGO", "number": "68" },
                { "count": 1, "name": "Town Store", "set": "OBF", "number": "196" },
            ], "energy": [
                { "count": 8, "name": "Basic Darkness Energy", "set": "SVE", "number": "7" },
                { "count": 2, "name": "Basic Water Energy", "set": "SVE", "number": "3" },
            ]
        }
    },
    {
        "name": "Rinesh John",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "blank",
        "sprite2": "roaring-moon",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "karlokzaii",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "arceus",
        "sprite2": "giratina-origin",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "DevilErickson",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "lugia",
        "sprite2": "archeops",
         "decklist": {
            "pokemon": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "trainer": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ], "energy": [

                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "name": "Edwin Bak Seng Chang",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "arceus",
        "sprite2": "armarouge",
"decklist": {
            "pokemon": [

                {"count": 4, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Armarouge", "set": "PAF", "number": "15"},
                {"count": 1, "name": "Armarouge ex", "set": "PAR", "number": "27"},
                {"count": 2, "name": "Charcadet", "set": "PAF", "number": "14"},
                {"count": 1, "name": "Delphox V", "set": "LOR", "number": "27"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 1, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Ditto", "set": "MEW", "number": "132"},
                {"count": 1, "name": "Cobalion", "set": "SIT", "number": "126"},
                        ], "trainer": [

                            {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                            {"count": 2, "name": "Professor's Research", "set": "PAF", "number": "87"},
                            {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                            {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                            {"count": 4, "name": "Ultra Ball", "set": "SVI", "number": "196"},
                            {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                            {"count": 1, "name": "Escape Rope", "set": "BST", "number": "125"},
                            {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                            {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                            {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                            {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                            {"count": 3, "name": "Magma Basin", "set": "BRS", "number": "144"},
                            {"count": 1, "name": "Path to the Peak", "set": "CRE", "number": "148"},
                        ], "energy": [

                            {"count": 7, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                            {"count": 1, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                            {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                            {"count": 1, "name": "Luminous Energy", "set": "PAL", "number": "191"},
                        ]
        }
    },
    {
        "name": "Anselm Minjoot",
        "flag": "../../../assets/sprites/blank.png",
        "event": "2024 Malaysia Regional League #3",
        "sprite1": "miraidon",
        "sprite2": "flaaffy",
"decklist": {
            "pokemon": [

                {"count": 2, "name": "Miraidon ex", "set": "SVI", "number": "81"},
                {"count": 2, "name": "Flaaffy", "set": "EVS", "number": "55"},
                {"count": 2, "name": "Mareep", "set": "EVS", "number": "54"},
                {"count": 1, "name": "Squawkabilly ex", "set": "PAF", "number": "75"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Raikou V", "set": "BRS", "number": "48"},
                {"count": 1, "name": "Zapdos", "set": "PGO", "number": "29"},
                {"count": 1, "name": "Raichu V", "set": "BRS", "number": "45"},
                {"count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                        ], "trainer": [

                            {"count": 3, "name": "Professor's Research", "set": "PAF", "number": "88"},
                            {"count": 3, "name": "Judge", "set": "SVI", "number": "176"},
                            {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                            {"count": 1, "name": "Arven", "set": "OBF", "number": "186"},
                            {"count": 1, "name": "Peony", "set": "CRE", "number": "150"},
                            {"count": 4, "name": "Electric Generator", "set": "SVI", "number": "170"},
                            {"count": 3, "name": "Ultra Ball", "set": "SVI", "number": "196"},
                            {"count": 2, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                            {"count": 2, "name": "Escape Rope", "set": "BST", "number": "125"},
                            {"count": 2, "name": "Nest Ball", "set": "PAF", "number": "84"},
                            {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                            {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                            {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                            {"count": 2, "name": "Exp. Share", "set": "SVI", "number": "174"},
                            {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                            {"count": 3, "name": "Path to the Peak", "set": "CRE", "number": "148"},
                        ], "energy": [

                {"count": 14, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 1, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
            ]
        }
    },
]