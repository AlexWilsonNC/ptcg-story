let masters = [
    {
        "firstName": "Hikaru Kobiyama",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/lugia.png',
        sprite2: '../../../../assets/sprites/archeops.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "Lugia VSTAR", "set": "SIT", "number": "139" },
                { "count": 3, "name": "Lugia V", "set": "SIT", "number": "138" },
                { "count": 4, "name": "Archeops", "set": "SIT", "number": "147" },
                { "count": 3, "name": "Cinccino", "set": "TEF", "number": "137" },
                { "count": 3, "name": "Minccino", "set": "TEF", "number": "136" },
                { "count": 1, "name": "Minccino", "set": "BRS", "number": "124" },
                { "count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70" },
                { "count": 1, "name": "Bloodmoon Ursaluna ex", "set": "TWM", "number": "141" },
                { "count": 1, "name": "Lumineon V", "set": "BRS", "number": "40" },
                { "count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78" },
            ],
            "trainer": [
                { "count": 3, "name": "Professor's Research", "set": "PAF", "number": "87" },
                { "count": 3, "name": "Iono", "set": "PAF", "number": "80" },
                { "count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 1, "name": "Jacq", "set": "SVI", "number": "175" },
                { "count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91" },
                { "count": 4, "name": "Capturing Aroma", "set": "SIT", "number": "153" },
                { "count": 2, "name": "Great Ball", "set": "PAL", "number": "183" },
                { "count": 2, "name": "Jamming Tower", "set": "TWM", "number": "153" },
            ],
            "energy": [
                { "count": 4, "name": "Jet Energy", "set": "PAL", "number": "190" },
                { "count": 4, "name": "Gift Energy", "set": "LOR", "number": "171" },
                { "count": 4, "name": "Mist Energy", "set": "TEF", "number": "161" },
                { "count": 3, "name": "Double Turbo Energy", "set": "BRS", "number": "151" },
                { "count": 1, "name": "V Guard Energy", "set": "SIT", "number": "169" },
                { "count": 1, "name": "Legacy Energy", "set": "TWM", "number": "167" },
            ]
        }
    },
    {
        "firstName": "Yusuke Makino",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/raging-bolt.png',
        sprite2: '../../../../assets/sprites/ogerpon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                { "count": 4, "name": "Raging Bolt ex", "set": "TEF", "number": "123" },
                { "count": 4, "name": "Teal Mask Ogerpon ex", "set": "TWM", "number": "25" },
                { "count": 1, "name": "Squawkabilly ex", "set": "PAF", "number": "75" },
                { "count": 1, "name": "Sandy Shocks", "set": "TEF", "number": "98" },
                { "count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46" },
                { "count": 1, "name": "Iron Bundle", "set": "PAR", "number": "56" },
            ],
            "trainer": [
                { "count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170" },
                { "count": 2, "name": "Iono", "set": "PAF", "number": "80" },
                { "count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 4, "name": "Nest Ball", "set": "PAF", "number": "84" },
                { "count": 4, "name": "Earthen Vessel", "set": "PAR", "number": "163" },
                { "count": 3, "name": "Ultra Ball", "set": "PAF", "number": "91" },
                { "count": 3, "name": "Pokégear 3.0", "set": "SVI", "number": "186" },
                { "count": 2, "name": "Pokémon Catcher", "set": "SVI", "number": "187" },
                { "count": 2, "name": "Switch Cart", "set": "ASR", "number": "154" },
                { "count": 2, "name": "Energy Retrieval", "set": "SVI", "number": "171" },
                { "count": 1, "name": "Super Rod", "set": "PAL", "number": "188" },
                { "count": 1, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189" },
                { "count": 1, "name": "Pal Pad", "set": "SVI", "number": "182" },
                { "count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157" },
                { "count": 2, "name": "Bravery Charm", "set": "PAL", "number": "173" },
                { "count": 2, "name": "PokéStop", "set": "PGO", "number": "68" },
            ],
            "energy": [
                { "count": 7, "name": "Basic Grass Energy", "set": "SVE", "number": "1" },
                { "count": 3, "name": "Basic Lightning Energy", "set": "SVE", "number": "4" },
                { "count": 3, "name": "Basic Fighting Energy", "set": "SVE", "number": "6" },
            ]
        }
    },
    {
        "firstName": "Shuma Kikuchi",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/raging-bolt.png',
        sprite2: '../../../../assets/sprites/ogerpon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                { "count": 4, "name": "Raging Bolt ex", "set": "TEF", "number": "123" },
                { "count": 4, "name": "Teal Mask Ogerpon ex", "set": "TWM", "number": "25" },
                { "count": 1, "name": "Squawkabilly ex", "set": "PAF", "number": "75" },
                { "count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46" },
                { "count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78" },
            ],
            "trainer": [
                { "count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170" },
                { "count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 4, "name": "Nest Ball", "set": "PAF", "number": "84" },
                { "count": 4, "name": "Trekking Shoes", "set": "CRZ", "number": "145" },
                { "count": 3, "name": "Earthen Vessel", "set": "PAR", "number": "163" },
                { "count": 3, "name": "Energy Retrieval", "set": "SVI", "number": "171" },
                { "count": 2, "name": "Ultra Ball", "set": "PAF", "number": "91" },
                { "count": 2, "name": "Pokégear 3.0", "set": "SVI", "number": "186" },
                { "count": 2, "name": "Switch Cart", "set": "ASR", "number": "154" },
                { "count": 1, "name": "Pokémon Catcher", "set": "SVI", "number": "187" },
                { "count": 1, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189" },
                { "count": 1, "name": "Pal Pad", "set": "SVI", "number": "182" },
                { "count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157" },
                { "count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135" },
                { "count": 3, "name": "Bravery Charm", "set": "PAL", "number": "173" },
                { "count": 2, "name": "PokéStop", "set": "PGO", "number": "68" },
            ],
            "energy": [
                { "count": 7, "name": "Basic Grass Energy", "set": "SVE", "number": "1" },
                { "count": 3, "name": "Basic Lightning Energy", "set": "SVE", "number": "4" },
                { "count": 3, "name": "Basic Fighting Energy", "set": "SVE", "number": "6" },
            ]
        }
    },
    {
        "firstName": "Ryoya Kurashima",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/lugia.png',
        sprite2: '../../../../assets/sprites/archeops.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "Lugia VSTAR", "set": "SIT", "number": "139" },
                { "count": 3, "name": "Lugia V", "set": "SIT", "number": "138" },
                { "count": 4, "name": "Archeops", "set": "SIT", "number": "147" },
                { "count": 2, "name": "Cinccino", "set": "TEF", "number": "137" },
                { "count": 2, "name": "Minccino", "set": "TEF", "number": "136" },
                { "count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70" },
                { "count": 1, "name": "Bloodmoon Ursaluna ex", "set": "TWM", "number": "141" },
                { "count": 1, "name": "Lumineon V", "set": "BRS", "number": "40" },
                { "count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20" },
                { "count": 1, "name": "Snorlax", "set": "PGO", "number": "55" },
            ],
            "trainer": [
                { "count": 3, "name": "Professor's Research", "set": "PAF", "number": "87" },
                { "count": 2, "name": "Iono", "set": "PAF", "number": "80" },
                { "count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172" },
                { "count": 1, "name": "Jacq", "set": "SVI", "number": "175" },
                { "count": 1, "name": "Carmine", "set": "TWM", "number": "145" },
                { "count": 1, "name": "Serena", "set": "SIT", "number": "164" },
                { "count": 1, "name": "Thorton", "set": "LOR", "number": "167" },
                { "count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91" },
                { "count": 4, "name": "Capturing Aroma", "set": "SIT", "number": "153" },
                { "count": 2, "name": "Great Ball", "set": "PAL", "number": "183" },
                { "count": 1, "name": "Nest Ball", "set": "PAF", "number": "84" },
                { "count": 1, "name": "Jamming Tower", "set": "TWM", "number": "153" },
                { "count": 1, "name": "Mesagoza", "set": "SVI", "number": "178" },
            ],
            "energy": [
                { "count": 4, "name": "Jet Energy", "set": "PAL", "number": "190" },
                { "count": 4, "name": "Gift Energy", "set": "LOR", "number": "171" },
                { "count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151" },
                { "count": 3, "name": "Mist Energy", "set": "TEF", "number": "161" },
                { "count": 1, "name": "Luminous Energy", "set": "PAL", "number": "191" },
                { "count": 1, "name": "Legacy Energy", "set": "TWM", "number": "167" },
            ]
        }
    },
    {
        "firstName": "Ryo Ishii",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/lugia.png',
        sprite2: '../../../../assets/sprites/archeops.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {"pokemon":[{"count":3,"name":"Lugia V","set":"SIT","number":138},{"count":3,"name":"Lugia VSTAR","set":"SIT","number":139},{"count":4,"name":"Archeops","set":"SIT","number":147},{"count":3,"name":"Minccino","set":"TEF","number":136},{"count":1,"name":"Minccino","set":"BRS","number":124},{"count":3,"name":"Cinccino","set":"TEF","number":137},{"count":1,"name":"Bloodmoon Ursaluna ex","set":"TWM","number":141},{"count":1,"name":"Iron Hands ex","set":"PAR","number":70},{"count":1,"name":"Lumineon V","set":"BRS","number":40}],"trainer":[{"count":3,"name":"Professor's Research","set":"PAF","number":88},{"count":3,"name":"Iono","set":"PAF","number":80},{"count":3,"name":"Boss's Orders","set":"PAL","number":172},{"count":1,"name":"Jacq","set":"SVI","number":175},{"count":1,"name":"Serena","set":"SIT","number":164},{"count":1,"name":"Roseanne's Backup","set":"BRS","number":148},{"count":4,"name":"Ultra Ball","set":"PAF","number":91},{"count":4,"name":"Capturing Aroma","set":"SIT","number":153},{"count":2,"name":"Great Ball","set":"PAL","number":183},{"count":2,"name":"Mesagoza","set":"SVI","number":178}],"energy":[{"count":4,"name":"Gift Energy","set":"LOR","number":171},{"count":4,"name":"Jet Energy","set":"PAL","number":190},{"count":4,"name":"Mist Energy","set":"TEF","number":161},{"count":3,"name":"Double Turbo Energy","set":"BRS","number":151},{"count":1,"name":"Legacy Energy","set":"TWM","number":167}]}
    },
    {
        "firstName": "Keiju Miya",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/lugia.png',
        sprite2: '../../../../assets/sprites/archeops.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {"pokemon":[{"count":3,"name":"Lugia V","set":"SIT","number":138},{"count":3,"name":"Lugia VSTAR","set":"SIT","number":139},{"count":4,"name":"Archeops","set":"SIT","number":147},{"count":2,"name":"Minccino","set":"TEF","number":136},{"count":1,"name":"Minccino","set":"BRS","number":124},{"count":3,"name":"Cinccino","set":"TEF","number":137},{"count":1,"name":"Bloodmoon Ursaluna ex","set":"TWM","number":141},{"count":1,"name":"Iron Hands ex","set":"PAR","number":70},{"count":1,"name":"Lumineon V","set":"BRS","number":40}],"trainer":[{"count":3,"name":"Professor's Research","set":"PAF","number":88},{"count":3,"name":"Iono","set":"PAF","number":80},{"count":3,"name":"Boss's Orders","set":"PAL","number":172},{"count":1,"name":"Jacq","set":"SVI","number":175},{"count":1,"name":"Serena","set":"SIT","number":164},{"count":4,"name":"Ultra Ball","set":"PAF","number":91},{"count":4,"name":"Capturing Aroma","set":"SIT","number":153},{"count":2,"name":"Nest Ball","set":"PAF","number":84},{"count":2,"name":"Mesagoza","set":"SVI","number":178},{"count":1,"name":"Jamming Tower","set":"TWM","number":153}],"energy":[{"count":4,"name":"Gift Energy","set":"LOR","number":171},{"count":4,"name":"Jet Energy","set":"PAL","number":190},{"count":4,"name":"Mist Energy","set":"TEF","number":161},{"count":3,"name":"Double Turbo Energy","set":"BRS","number":151},{"count":1,"name":"V Guard Energy","set":"SIT","number":169},{"count":1,"name":"Legacy Energy","set":"TWM","number":167}]}
    },
    {
        "firstName": "Takayuki Nakamura",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/roaring-moon.png',
        sprite2: '../../../../assets/sprites/darkrai.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {"pokemon":[{"count":3,"name":"Roaring Moon ex","set":"PAR","number":124},{"count":1,"name":"Roaring Moon","set":"TEF","number":109},{"count":1,"name":"Darkrai V","set":"ASR","number":98},{"count":1,"name":"Darkrai VSTAR","set":"ASR","number":99},{"count":1,"name":"Squawkabilly ex","set":"PAF","number":75},{"count":1,"name":"Mew ex","set":"MEW","number":151},{"count":1,"name":"Lumineon V","set":"BRS","number":40},{"count":1,"name":"Radiant Greninja","set":"ASR","number":46},{"count":1,"name":"Iron Bundle","set":"PAR","number":56},{"count":1,"name":"Flutter Mane","set":"TEF","number":78}],"trainer":[{"count":4,"name":"Professor Sada's Vitality","set":"PAR","number":170},{"count":1,"name":"Boss's Orders","set":"PAL","number":172},{"count":4,"name":"Dark Patch","set":"ASR","number":139},{"count":4,"name":"Trekking Shoes","set":"CRZ","number":145},{"count":4,"name":"Nest Ball","set":"PAF","number":84},{"count":4,"name":"Ultra Ball","set":"PAF","number":91},{"count":4,"name":"Energy Switch","set":"SVI","number":173},{"count":3,"name":"Earthen Vessel","set":"PAR","number":163},{"count":2,"name":"Switch Cart","set":"ASR","number":154},{"count":1,"name":"Prime Catcher","set":"TEF","number":157},{"count":1,"name":"Canceling Cologne","set":"ASR","number":136},{"count":1,"name":"Super Rod","set":"PAL","number":188},{"count":1,"name":"Pal Pad","set":"SVI","number":182},{"count":4,"name":"PokéStop","set":"PGO","number":68}],"energy":[{"count":7,"name":"Darkness","set":"Energy","number":7},{"count":3,"name":"Water","set":"Energy","number":3}]}
    },
    {
        "firstName": "Shinji Mori",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/gardevoir.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Gardevoir ex", "set": "SVI", "number": "86"},
                {"count": 4, "name": "Kirlia", "set": "SIT", "number": "68"},
                {"count": 4, "name": "Ralts", "set": "ASR", "number": "60"},
                {"count": 2, "name": "Munkidori", "set": "TWM", "number": "95"},
                {"count": 1, "name": "Drifloon", "set": "SVI", "number": "89"},
                {"count": 1, "name": "Scream Tail", "set": "PR-SV", "number": "65"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Cresselia", "set": "LOR", "number": "74"},
                {"count": 1, "name": "Klefki", "set": "SVI", "number": "96"},
                {"count": 1, "name": "Tatsugiri", "set": "TWM", "number": "131"},
                        ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 2, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 2, "name": "Ultra Ball", "set": "SVI", "number": "196"},
                {"count": 2, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Unfair Stamp", "set": "TWM", "number": "165"},
                {"count": 2, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 2, "name": "Technical Machine: Evolution", "set": "PAR", "number": "178"},
                {"count": 2, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 8, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 2, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
            ]
        }
    },
    {
        "firstName": "Yuto Nakajima",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Daiki Kawakami",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Hirofumi Fujimoto",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Masato Tao",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Boku Kiwada",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Kohei Murakawa",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Shinya Karashima",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Takumi Sugiura",
        flag: '../../../../assets/flags/japan.png',
        "event": "2024 Japan Championships",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Origin Forme Dialga VSTAR", "set": "ASR", "number": "114"},
                {"count": 3, "name": "Origin Forme Dialga V", "set": "ASR", "number": "113"},
                {"count": 4, "name": "Metang", "set": "TEF", "number": "114"},
                {"count": 4, "name": "Beldum", "set": "TEF", "number": "113"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor's Research", "set": "PAF", "number": "87"},
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Jamming Tower", "set": "TWM", "number": "153"},
            ],
            "energy": [
                {"count": 14, "name": "Basic Metal Energy", "set": "SVE", "number": "8"},
            ]
        }
    },
]