let masters = [
    {
        "firstName": "Tord Reklev",
        flag: '../../../../assets/flags/norway.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Charizard ex", "set": "PAF", "number": "54"},
                {"count": 1, "name": "Charmeleon", "set": "PAF", "number": "8"},
                {"count": 4, "name": "Charmander", "set": "OBF", "number": "26"},
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 1, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Lumineon V", "set": "BRS", "number": "40"},
                {"count": 1, "name": "Cleffa", "set": "OBF", "number": "80"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
            ],
            "trainer": [
                {"count": 3, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 2, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 1, "name": "Roxanne", "set": "ASR", "number": "150"},
                {"count": 1, "name": "Team Yell's Cheer", "set": "BRS", "number": "149"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
            ]
        }
    },
    {
        "firstName": "Isaiah Bradner",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/giratina-origin.png',
        sprite2: '../../../../assets/sprites/comfey.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Giratina VSTAR", "set": "LOR", "number": "131"},
                {"count": 3, "name": "Giratina V", "set": "LOR", "number": "130"},
                {"count": 4, "name": "Comfey", "set": "LOR", "number": "79"},
                {"count": 1, "name": "Banette ex", "set": "SVI", "number": "88"},
                {"count": 1, "name": "Banette", "set": "LOR", "number": "73"},
                {"count": 2, "name": "Shuppet", "set": "SVI", "number": "87"},
                {"count": 1, "name": "Cramorant", "set": "LOR", "number": "50"},
                {"count": 1, "name": "Sableye", "set": "LOR", "number": "70"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Iron Leaves ex", "set": "TEF", "number": "25"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
              ],
            "trainer": [
                {"count": 4, "name": "Colress's Experiment", "set": "LOR", "number": "155"},
                {"count": 1, "name": "Roxanne", "set": "ASR", "number": "150"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Mirage Gate", "set": "LOR", "number": "163"},
                {"count": 3, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 2, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 1, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Artazon", "set": "PAF", "number": "76"},
                {"count": 1, "name": "Temple of Sinnoh", "set": "ASR", "number": "155"},
            ],
            "energy": [
                {"count": 5, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 4, "name": "Basic Grass Energy", "set": "SVE", "number": "1"},
                {"count": 4, "name": "Jet Energy", "set": "PAL", "number": "190"},          
            ]
        }
    },
    {
        "firstName": "Alessandro Cremascoli",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Noivern ex", "set": "PAF", "number": "69"},
                {"count": 1, "name": "Noibat", "set": "PAF", "number": "68"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Luxray V", "set": "ASR", "number": "50"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Chi-Yu ex", "set": "PAL", "number": "40"},
                {"count": 1, "name": "Bouffalant", "set": "LOR", "number": "148"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Mawile", "set": "LOR", "number": "71"},
                    ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 1, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Cyllene", "set": "ASR", "number": "138"},
                {"count": 1, "name": "Thorton", "set": "LOR", "number": "167"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Roseanne's Backup", "set": "BRS", "number": "148"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 4, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 3, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 3, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 2, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 2, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 1, "name": "Switch Cart", "set": "ASR", "number": "154"},
                {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 2, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 1, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 2, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 2, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                {"count": 1, "name": "Mist Energy", "set": "TEF", "number": "161"},    
            ]
        }
    },
    {
        "firstName": "William Azevedo",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Charizard ex", "set": "PAF", "number": "54"},
                {"count": 1, "name": "Charmeleon", "set": "PAF", "number": "8"},
                {"count": 3, "name": "Charmander", "set": "OBF", "number": "26"},
                {"count": 1, "name": "Charmander", "set": "PAF", "number": "7"},
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Lumineon V", "set": "BRS", "number": "40"},
                {"count": 1, "name": "Cleffa", "set": "OBF", "number": "80"},
                                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
            ],
            "trainer": [
                {"count": 3, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Roxanne", "set": "ASR", "number": "150"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 3, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 2, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
            ]
        }
    },
    {
        "firstName": "Uffe Axelsen",
        flag: '../../../../assets/flags/denmark.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/iron-hands.png',
        sprite2: '../../../../assets/sprites/iron-crown.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Iron Crown ex", "set": "TEF", "number": "81"},
                {"count": 3, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 2, "name": "Miraidon", "set": "TEF", "number": "121"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor's Research", "set": "PAF", "number": "88"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Erika's Invitation", "set": "MEW", "number": "160"},
                {"count": 4, "name": "Electric Generator", "set": "PAF", "number": "79"},
                {"count": 4, "name": "Techno Radar", "set": "PAR", "number": "180"},
                {"count": 2, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch Cart", "set": "ASR", "number": "154"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 4, "name": "Future Booster Energy Capsule", "set": "TEF", "number": "149"},
                {"count": 2, "name": "Heavy Baton", "set": "TEF", "number": "151"},
            ],
            "energy": [
                {"count": 14, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 2, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
            ]
        }
    },
    {
        "firstName": "Juho Kallama",
        flag: '../../../../assets/flags/finland.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/iron-hands.png',
        sprite2: '../../../../assets/sprites/iron-crown.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 4, "name": "Iron Crown ex", "set": "TEF", "number": "81"},
                {"count": 2, "name": "Miraidon", "set": "TEF", "number": "121"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor's Research", "set": "PAF", "number": "88"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Electric Generator", "set": "PAF", "number": "79"},
                {"count": 4, "name": "Techno Radar", "set": "PAR", "number": "180"},
                {"count": 3, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 2, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 4, "name": "Future Booster Energy Capsule", "set": "TEF", "number": "149"},
                {"count": 2, "name": "Heavy Baton", "set": "TEF", "number": "151"},
                {"count": 1, "name": "Technical Machine: Crisis Punch", "set": "PAF", "number": "90"},
                {"count": 1, "name": "Town Store", "set": "OBF", "number": "196"},
            ],
            "energy": [
                {"count": 14, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 1, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
            ]
        }
    },
    {
        "firstName": "Mark Høstrup",
        flag: '../../../../assets/flags/denmark.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/roaring-moon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Roaring Moon", "set": "TEF", "number": "109"},
                {"count": 3, "name": "Roaring Moon ex", "set": "PAR", "number": "124"},
                {"count": 3, "name": "Dudunsparce", "set": "TEF", "number": "129"},
                {"count": 3, "name": "Dunsparce", "set": "TEF", "number": "128"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170"},
                {"count": 4, "name": "Explorer's Guidance", "set": "TEF", "number": "147"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 4, "name": "Dark Patch", "set": "ASR", "number": "139"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 3, "name": "Trekking Shoes", "set": "CRZ", "number": "145"},
                {"count": 2, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 4, "name": "Ancient Booster Energy Capsule", "set": "TEF", "number": "140"},
                {"count": 3, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 9, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
            ]
        }
    },
    {
        "firstName": "Bert Wolters",
        flag: '../../../../assets/flags/netherlands.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/pidgeot.png',
        sprite2: '../../../../assets/sprites/snorlax.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 3, "name": "Snorlax", "set": "PGO", "number": "55"},
                {"count": 1, "name": "Luxray V", "set": "ASR", "number": "50"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Chi-Yu ex", "set": "PAL", "number": "40"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Mawile", "set": "LOR", "number": "71"},
                        ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 2, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 1, "name": "Miss Fortune Sisters", "set": "LOR", "number": "164"},
                {"count": 1, "name": "Giacomo", "set": "PAL", "number": "182"},
                {"count": 1, "name": "Erika's Invitation", "set": "MEW", "number": "160"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Cyllene", "set": "ASR", "number": "138"},
                {"count": 1, "name": "Team Yell's Cheer", "set": "BRS", "number": "149"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 3, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 2, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 2, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 2, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 2, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Switch Cart", "set": "ASR", "number": "154"},
                {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 1, "name": "Artazon", "set": "PAF", "number": "76"},
                {"count": 1, "name": "Magma Basin", "set": "BRS", "number": "144"},
                {"count": 1, "name": "Temple of Sinnoh", "set": "ASR", "number": "155"},
            ],
            "energy": [
                {"count": 2, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 2, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
            ]
        }
    },
    {
        "firstName": "Fabien Pujol",
        flag: '../../../../assets/flags/france.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/gardevoir.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Gardevoir ex", "set": "PAF", "number": "29"},
                {"count": 4, "name": "Kirlia", "set": "SIT", "number": "68"},
                {"count": 3, "name": "Ralts", "set": "ASR", "number": "60"},
                {"count": 1, "name": "Ralts", "set": "SIT", "number": "67"},
                {"count": 1, "name": "Drifloon", "set": "SVI", "number": "89"},
                {"count": 1, "name": "Scream Tail", "set": "PAR", "number": "86"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Klefki", "set": "SVI", "number": "96"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                           ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 3, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 3, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 2, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 2, "name": "Technical Machine: Evolution", "set": "PAR", "number": "178"},
                {"count": 1, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Luxurious Cape", "set": "PAR", "number": "166"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 1, "name": "Town Store", "set": "OBF", "number": "196"},
                        ],
            "energy": [
                {"count": 9, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},       
            ]
        }
    },
    {
        "firstName": "Simone Soldo",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "João Pedro Medeiros",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/koraidon.png',
        sprite2: '../../../../assets/sprites/roaring-moon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Roaring Moon", "set": "TEF", "number": "109"},
                {"count": 4, "name": "Koraidon", "set": "TEF", "number": "119"},
                {"count": 4, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                ],
            "trainer": [
                {"count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170"},
                {"count": 4, "name": "Explorer's Guidance", "set": "TEF", "number": "147"},
                {"count": 1, "name": "Boss's Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Awakening Drum", "set": "TEF", "number": "141"},
                {"count": 4, "name": "Ancient Booster Energy Capsule", "set": "TEF", "number": "140"},
                {"count": 3, "name": "PokéStop", "set": "PGO", "number": "68"},
            ],
            "energy": [
                {"count": 7, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
                {"count": 1, "name": "Basic Fighting Energy", "set": "SVE", "number": "6"},
            ]
        }
    },
    {
        "firstName": "Pedro Eugenio Torres",
        flag: '../../../../assets/flags/spain.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/comfey.png',
        sprite2: '../../../../assets/sprites/roaring-moon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Comfey", "set": "LOR", "number": "79"},
                {"count": 1, "name": "Cramorant", "set": "LOR", "number": "50"},
                {"count": 1, "name": "Sableye", "set": "LOR", "number": "70"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Roaring Moon ex", "set": "PAR", "number": "124"},
                {"count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 1, "name": "Raikou V", "set": "BRS", "number": "48"},
                {"count": 1, "name": "Hoopa ex", "set": "PAR", "number": "98"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Iron Bundle", "set": "PAR", "number": "56"},
                   ],
            "trainer": [
                {"count": 4, "name": "Colress's Experiment", "set": "LOR", "number": "155"},
                {"count": 1, "name": "Roxanne", "set": "ASR", "number": "150"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Mirage Gate", "set": "LOR", "number": "163"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Switch Cart", "set": "ASR", "number": "154"},
                {"count": 4, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 3, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 3, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Rescue Board", "set": "TEF", "number": "159"},
                {"count": 2, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 4, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
                {"count": 3, "name": "Basic Water Energy", "set": "SVE", "number": "3"},
                {"count": 2, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 1, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},     
            ]
        }
    },
    {
        "firstName": "George Giannoulidis",
        flag: '../../../../assets/flags/greece.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Charizard ex", "set": "PAF", "number": "54"},
                {"count": 1, "name": "Charmeleon", "set": "PAF", "number": "8"},
                {"count": 2, "name": "Charmander", "set": "OBF", "number": "26"},
                {"count": 2, "name": "Charmander", "set": "PR-SV", "number": "47"},
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 2, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
            ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Roxanne", "set": "ASR", "number": "150"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 1, "name": "Gift Energy", "set": "LOR", "number": "171"},
            ]
        }
    },
    {
        "firstName": "Azul Garcia Griego",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/giratina-origin.png',
        sprite2: '../../../../assets/sprites/comfey.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Solomon Shurge",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/iron-hands.png',
        sprite2: '../../../../assets/sprites/iron-crown.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Iron Crown ex", "set": "TEF", "number": "81"},
                {"count": 3, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 2, "name": "Miraidon", "set": "TEF", "number": "121"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Iron Bundle", "set": "PAR", "number": "56"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor's Research", "set": "PAF", "number": "88"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Electric Generator", "set": "PAF", "number": "79"},
                {"count": 4, "name": "Techno Radar", "set": "PAR", "number": "180"},
                {"count": 3, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 4, "name": "Future Booster Energy Capsule", "set": "TEF", "number": "149"},
                {"count": 2, "name": "Heavy Baton", "set": "TEF", "number": "151"},
                {"count": 1, "name": "Town Store", "set": "OBF", "number": "196"},
            ],
            "energy": [
                {"count": 14, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 1, "name": "Basic Water Energy", "set": "SVE", "number": "3"},
                {"count": 1, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 1, "name": "Gift Energy", "set": "LOR", "number": "171"},
            ]
        }
    },
    {
        "firstName": "Andres David Torres",
        flag: '../../../../assets/flags/colombia.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/roaring-moon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Roaring Moon", "set": "TEF", "number": "109"},
                {"count": 2, "name": "Roaring Moon ex", "set": "PAR", "number": "124"},
                {"count": 3, "name": "Dudunsparce", "set": "TEF", "number": "129"},
                {"count": 3, "name": "Dunsparce", "set": "TEF", "number": "128"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Iron Bundle", "set": "PAR", "number": "56"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170"},
                {"count": 4, "name": "Explorer's Guidance", "set": "TEF", "number": "147"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Thorton", "set": "LOR", "number": "167"},
                {"count": 4, "name": "Dark Patch", "set": "ASR", "number": "139"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 3, "name": "Trekking Shoes", "set": "CRZ", "number": "145"},
                {"count": 2, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 4, "name": "Ancient Booster Energy Capsule", "set": "TEF", "number": "140"},
                {"count": 3, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 9, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
            ]
        }
    },
    {
        "firstName": "Julian Gort-Barry",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/arceus.png',
        sprite2: '../../../../assets/sprites/armarouge.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Armarouge", "set": "PAF", "number": "15"},
                {"count": 1, "name": "Armarouge ex", "set": "PAR", "number": "27"},
                {"count": 3, "name": "Charcadet", "set": "PAF", "number": "14"},
                {"count": 1, "name": "Delphox V", "set": "LOR", "number": "27"},
                {"count": 1, "name": "Gouging Fire ex", "set": "TEF", "number": "38"},
                {"count": 1, "name": "Radiant Heatran", "set": "ASR", "number": "27"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Squawkabilly ex", "set": "PAF", "number": "75"},
                {"count": 1, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 1, "name": "Bidoof", "set": "CRZ", "number": "111"},
            ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 3, "name": "Professor's Research", "set": "PAF", "number": "87"},
                {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 2, "name": "Capturing Aroma", "set": "SIT", "number": "153"},
                {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
                {"count": 4, "name": "Magma Basin", "set": "BRS", "number": "144"},
            ],
            "energy": [
                {"count": 9, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},            
            ]
        }
    },
    {
        "firstName": "Piper Lepine",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/gardevoir.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Gardevoir ex", "set": "PAF", "number": "29"},
                {"count": 1, "name": "Gallade", "set": "ASR", "number": "62"},
                {"count": 4, "name": "Kirlia", "set": "SIT", "number": "68"},
                {"count": 4, "name": "Ralts", "set": "ASR", "number": "60"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Drifloon", "set": "SVI", "number": "89"},
                {"count": 1, "name": "Scream Tail", "set": "PAR", "number": "86"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                           ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Professor's Research", "set": "PAF", "number": "87"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 3, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 1, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 1, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 2, "name": "Technical Machine: Evolution", "set": "PAR", "number": "178"},
                {"count": 1, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Luxurious Cape", "set": "PAR", "number": "166"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
            ],
            "energy": [
                {"count": 8, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},             ]
        }
    },
    {
        "firstName": "Caleb Gedemer",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/giratina-origin.png',
        sprite2: '../../../../assets/sprites/comfey.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Tom Hellkamp",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Simon Tschan",
        flag: '../../../../assets/flags/switzerland.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Ryan Miller",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/chien-pao.png',
        sprite2: '../../../../assets/sprites/baxcalibur.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Chien-Pao ex", "set": "PAL", "number": "61"},
                {"count": 2, "name": "Baxcalibur", "set": "PAL", "number": "60"},
                {"count": 2, "name": "Frigibax", "set": "PAL", "number": "57"},
                {"count": 1, "name": "Frigibax", "set": "PAF", "number": "17"},
                {"count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                   ],
            "trainer": [
                {"count": 4, "name": "Irida", "set": "ASR", "number": "147"},
                {"count": 2, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Canceling Cologne", "set": "ASR", "number": "136"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 2, "name": "PokéStop", "set": "PGO", "number": "68"},
            ],
            "energy": [
                {"count": 8, "name": "Basic Water Energy", "set": "SVE", "number": "3"},
                {"count": 1, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},     
            ]
        }
    },
    {
        "firstName": "Stéphane Ivanoff",
        flag: '../../../../assets/flags/france.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/gardevoir.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Gardevoir ex", "set": "PAF", "number": "29"},
                {"count": 4, "name": "Kirlia", "set": "SIT", "number": "68"},
                {"count": 4, "name": "Ralts", "set": "ASR", "number": "60"},
                {"count": 1, "name": "Drifloon", "set": "SVI", "number": "89"},
                {"count": 1, "name": "Scream Tail", "set": "PAR", "number": "86"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Klefki", "set": "SVI", "number": "96"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                           ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 3, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 3, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 2, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 2, "name": "Technical Machine: Evolution", "set": "PAR", "number": "178"},
                {"count": 1, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Luxurious Cape", "set": "PAR", "number": "166"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
            ],
            "energy": [
                {"count": 9, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},       
                  ]
        }
    },
    {
        "firstName": "Christian Graziano",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Aidan Khus",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Charizard ex", "set": "PAF", "number": "54"},
                {"count": 1, "name": "Charmeleon", "set": "PAF", "number": "8"},
                {"count": 2, "name": "Charmander", "set": "OBF", "number": "26"},
                {"count": 2, "name": "Charmander", "set": "PAF", "number": "7"},
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Regieleki", "set": "ASR", "number": "51"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
            ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
                {"count": 1, "name": "Temple of Sinnoh", "set": "ASR", "number": "155"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 1, "name": "Mist Energy", "set": "TEF", "number": "161"},            
            ]
        }
    },
    {
        "firstName": "Braiden Elfert",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/espathra.png',
        sprite2: '../../../../assets/sprites/xatu.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Espathra ex", "set": "PAF", "number": "6"},
                {"count": 3, "name": "Flittle", "set": "PAF", "number": "41"},
                {"count": 3, "name": "Xatu", "set": "PAF", "number": "26"},
                {"count": 3, "name": "Natu", "set": "PAF", "number": "25"},
                {"count": 1, "name": "Banette ex", "set": "SVI", "number": "88"},
                {"count": 1, "name": "Banette", "set": "LOR", "number": "73"},
                {"count": 2, "name": "Shuppet", "set": "SVI", "number": "87"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Cleffa", "set": "OBF", "number": "80"},
                {"count": 1, "name": "Radiant Alakazam", "set": "SIT", "number": "59"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
                        ],
            "trainer": [
                {"count": 4, "name": "Professor's Research", "set": "PAF", "number": "88"},
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Capturing Aroma", "set": "SIT", "number": "153"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Energy Retrieval", "set": "SVI", "number": "171"},
                {"count": 1, "name": "Rigid Band", "set": "MEW", "number": "165"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 2, "name": "Pokémon League Headquarters", "set": "OBF", "number": "192"},
            ],
            "energy": [
                {"count": 11, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
            ]
        }
    },
    {
        "firstName": "Brent Tonisson",
        flag: '../../../../assets/flags/australia.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Charizard ex", "set": "PAF", "number": "54"},
                {"count": 1, "name": "Charmeleon", "set": "PAF", "number": "8"},
                {"count": 2, "name": "Charmander", "set": "OBF", "number": "26"},
                {"count": 2, "name": "Charmander", "set": "PAF", "number": "7"},
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Regieleki", "set": "ASR", "number": "51"},
                {"count": 1, "name": "Manaphy", "set": "BRS", "number": "41"},
            ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Collapsed Stadium", "set": "BRS", "number": "137"},
                {"count": 1, "name": "Temple of Sinnoh", "set": "ASR", "number": "155"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 1, "name": "Mist Energy", "set": "TEF", "number": "161"},            
            ]
        }
    },
    {
        "firstName": "Alessandro Frenda",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/arceus.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 3, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Slaking V", "set": "PGO", "number": "58"},
                {"count": 1, "name": "Luxray V", "set": "ASR", "number": "50"},
                {"count": 1, "name": "Mew ex", "set": "MEW", "number": "151"},
                {"count": 1, "name": "Radiant Gardevoir", "set": "LOR", "number": "69"},
                {"count": 1, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Spiritomb", "set": "PAL", "number": "89"},
                {"count": 1, "name": "Skwovet", "set": "SVI", "number": "151"},
                        ],
            "trainer": [
                {"count": 3, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Professor's Research", "set": "PAF", "number": "87"},
                {"count": 1, "name": "Cheren's Care", "set": "BRS", "number": "134"},
                {"count": 1, "name": "Roseanne's Backup", "set": "BRS", "number": "148"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 3, "name": "Grabber", "set": "MEW", "number": "162"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Technical Machine: Devolution", "set": "PAR", "number": "177"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
            ],
            "energy": [
                {"count": 4, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},
                {"count": 2, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                {"count": 2, "name": "Jet Energy", "set": "PAL", "number": "190"},
                {"count": 1, "name": "Mist Energy", "set": "TEF", "number": "161"},
            ]
        }
    },
    {
        "firstName": "Fabrizio Inga Silva",
        flag: '../../../../assets/flags/peru.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/chien-pao.png',
        sprite2: '../../../../assets/sprites/baxcalibur.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Chien-Pao ex", "set": "PAL", "number": "61"},
                {"count": 2, "name": "Baxcalibur", "set": "PAL", "number": "60"},
                {"count": 3, "name": "Frigibax", "set": "PAL", "number": "57"},
                {"count": 1, "name": "Iron Hands ex", "set": "PAR", "number": "70"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                {"count": 1, "name": "Iron Bundle", "set": "PAR", "number": "56"},        
                      ],
            "trainer": [
                {"count": 4, "name": "Irida", "set": "ASR", "number": "147"},
                {"count": 2, "name": "Ciphermaniac's Codebreaking", "set": "TEF", "number": "145"},
                {"count": 1, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 4, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189"},
                {"count": 3, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 3, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Canceling Cologne", "set": "ASR", "number": "136"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Prime Catcher", "set": "TEF", "number": "157"},
                {"count": 2, "name": "PokéStop", "set": "PGO", "number": "68"},
            ],
            "energy": [
                {"count": 8, "name": "Basic Water Energy", "set": "SVE", "number": "3"},
                {"count": 1, "name": "Basic Lightning Energy", "set": "SVE", "number": "4"},     
            ]
        }
    },
    {
        "firstName": "Samuel Gay",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/pidgeot.png',
        sprite2: '../../../../assets/sprites/snorlax.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 2, "name": "Pidgeot ex", "set": "OBF", "number": "164"},
                {"count": 2, "name": "Pidgey", "set": "MEW", "number": "16"},
                {"count": 2, "name": "Snorlax", "set": "PGO", "number": "55"},
                {"count": 1, "name": "Luxray V", "set": "ASR", "number": "50"},
                {"count": 1, "name": "Rotom V", "set": "CRZ", "number": "45"},
                {"count": 1, "name": "Klawf ex", "set": "OBF", "number": "120"},
                {"count": 1, "name": "Radiant Charizard", "set": "CRZ", "number": "20"},
                {"count": 1, "name": "Chi-Yu ex", "set": "PAL", "number": "40"},
                {"count": 1, "name": "Mimikyu", "set": "PAF", "number": "37"},
                {"count": 1, "name": "Mawile", "set": "LOR", "number": "71"},
                        ],
            "trainer": [
                {"count": 4, "name": "Arven", "set": "OBF", "number": "186"},
                {"count": 2, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 2, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 2, "name": "Cyllene", "set": "ASR", "number": "138"},
                {"count": 1, "name": "Giacomo", "set": "PAL", "number": "182"},
                {"count": 1, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Team Yell's Cheer", "set": "BRS", "number": "149"},
                {"count": 1, "name": "Miss Fortune Sisters", "set": "LOR", "number": "164"},
                {"count": 1, "name": "Erika's Invitation", "set": "MEW", "number": "160"},
                {"count": 1, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 1, "name": "Cheren's Care", "set": "BRS", "number": "134"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 2, "name": "Rare Candy", "set": "PAF", "number": "89"},
                {"count": 2, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 1, "name": "Hisuian Heavy Ball", "set": "ASR", "number": "146"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch Cart", "set": "ASR", "number": "154"},
                {"count": 1, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 2, "name": "Bravery Charm", "set": "PAL", "number": "173"},
                {"count": 1, "name": "Defiance Band", "set": "SVI", "number": "169"},
                {"count": 1, "name": "Defiance Vest", "set": "PAR", "number": "162"},
                {"count": 1, "name": "Forest Seal Stone", "set": "SIT", "number": "156"},
                {"count": 1, "name": "Hero's Cape", "set": "TEF", "number": "152"},
                {"count": 2, "name": "Temple of Sinnoh", "set": "ASR", "number": "155"},
                {"count": 1, "name": "Artazon", "set": "PAF", "number": "76"},
            ],
            "energy": [
                {"count": 2, "name": "Basic Fire Energy", "set": "SVE", "number": "2"},
                {"count": 2, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
            ]
        }
    },
    {
        "firstName": "Weifeng Lu",
        flag: '../../../../assets/flags/china.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Austin Drake",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    // {
    //     "firstName": "Allan Xu",
    //     flag: '../../../../assets/flags/canada.png',
    //     "event": "2024 EUIC",
    //     sprite1: '../../../../assets/sprites/blank.png',
    //     sprite2: '../../../../assets/sprites/hyphen.png',
    //     // list: '../../../../assets/list-icon.png',
    //     list: '../../../../assets/sprites/blank.png',
    //     "decklist": {
    //         "pokemon": [
    //             { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
    //         ],
    //         "trainer": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ],
    //         "energy": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ]
    //     }
    // },
    // {
    //     "firstName": "Hector Ibarra",
    //     flag: '../../../../assets/flags/usa.png',
    //     "event": "2024 EUIC",
    //     sprite1: '../../../../assets/sprites/blank.png',
    //     sprite2: '../../../../assets/sprites/hyphen.png',
    //     // list: '../../../../assets/list-icon.png',
    //     list: '../../../../assets/sprites/blank.png',
    //     "decklist": {
    //         "pokemon": [
    //             { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
    //         ],
    //         "trainer": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ],
    //         "energy": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ]
    //     }
    // },
    // {
    //     "firstName": "Hiromu Sasaki",
    //     flag: '../../../../assets/flags/japan.png',
    //     "event": "2024 EUIC",
    //     sprite1: '../../../../assets/sprites/giratina-origin.png',
    //     sprite2: '../../../../assets/sprites/comfey.png',
    //     // list: '../../../../assets/list-icon.png',
    //     list: '../../../../assets/sprites/blank.png',
    //     "decklist": {
    //         "pokemon": [
    //             {"count": 3, "name": "Giratina VSTAR", "set": "LOR", "number": "131"},
    //             {"count": 3, "name": "Giratina V", "set": "LOR", "number": "130"},
    //             {"count": 4, "name": "Comfey", "set": "LOR", "number": "79"},
    //             {"count": 1, "name": "Sableye", "set": "LOR", "number": "70"},
    //             {"count": 1, "name": "Cramorant", "set": "LOR", "number": "50"},
    //             {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
    //             {"count": 1, "name": "Iron Leaves ex", "set": "TEF", "number": "25"},
    //             {"count": 1, "name": "Spiritomb", "set": "PAL", "number": "89"},
    //             {"count": 4, "name": "Colress's Experiment", "set": "LOR", "number": "155"},
    //             {"count": 2, "name": "Roxanne", "set": "ASR", "number": "150"},
    //             {"count": 2, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
    //             {"count": 1, "name": "Iono", "set": "PAF", "number": "80"},
    //             {"count": 4, "name": "Mirage Gate", "set": "LOR", "number": "163"},
    //             {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
    //             {"count": 3, "name": "Super Rod", "set": "PAL", "number": "188"},
    //             {"count": 2, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},            ],
    //         "trainer": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ],
    //         "energy": [
    //             { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
    //         ]
    //     }
    // },
]

let seniors = [
    {
        "firstName": "Gabriel Fernandez",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/koraidon.png',
        sprite2: '../../../../assets/sprites/roaring-moon.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Roaring Moon", "set": "TEF", "number": "109"},
                {"count": 4, "name": "Koraidon", "set": "TEF", "number": "119"},
                {"count": 4, "name": "Flutter Mane", "set": "TEF", "number": "78"},
                {"count": 1, "name": "Radiant Greninja", "set": "ASR", "number": "46"},
                ],
            "trainer": [
                {"count": 4, "name": "Professor Sada's Vitality", "set": "PAR", "number": "170"},
                {"count": 4, "name": "Explorer's Guidance", "set": "TEF", "number": "147"},
                {"count": 1, "name": "Boss's Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Penny", "set": "SVI", "number": "183"},
                {"count": 4, "name": "Earthen Vessel", "set": "PAR", "number": "163"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 4, "name": "Pokégear 3.0", "set": "SVI", "number": "186"},
                {"count": 3, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 2, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 2, "name": "Super Rod", "set": "PAL", "number": "188"},
                {"count": 1, "name": "Superior Energy Retrieval", "set": "PAL", "number": "189"},
                {"count": 1, "name": "Pal Pad", "set": "SVI", "number": "182"},
                {"count": 1, "name": "Awakening Drum", "set": "TEF", "number": "141"},
                {"count": 4, "name": "Ancient Booster Energy Capsule", "set": "TEF", "number": "140"},
                {"count": 3, "name": "PokéStop", "set": "PGO", "number": "68"},
            ],
            "energy": [
                {"count": 6, "name": "Basic Darkness Energy", "set": "SVE", "number": "7"},
                {"count": 2, "name": "Basic Fighting Energy", "set": "SVE", "number": "6"},
            ]
        }
    },
    {
        "firstName": "Benny Billinger",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/charizard.png',
        sprite2: '../../../../assets/sprites/pidgeot.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Roman Garus",
        flag: '../../../../assets/flags/slovakia.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Alberto Zamuner",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Giovanny Sasso",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/arceus.png',
        sprite2: '../../../../assets/sprites/giratina-origin.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Giratina VSTAR", "set": "LOR", "number": "131"},
                {"count": 2, "name": "Giratina V", "set": "LOR", "number": "130"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Iron Leaves ex", "set": "TEF", "number": "25"},
                {"count": 1, "name": "Aerodactyl VSTAR", "set": "LOR", "number": "93"},
                {"count": 1, "name": "Aerodactyl V", "set": "LOR", "number": "92"},
                {"count": 1, "name": "Jirachi", "set": "PAR", "number": "126"},
                {"count": 1, "name": "Radiant Gardevoir", "set": "LOR", "number": "69"},
                {"count": 1, "name": "Skwovet", "set": "SVI", "number": "151"},
                 ],
            "trainer": [
                {"count": 4, "name": "Judge", "set": "SVI", "number": "176"},
                {"count": 3, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 1, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Cheren's Care", "set": "BRS", "number": "134"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
            ],
            "energy": [
                {"count": 5, "name": "Basic Grass Energy", "set": "SVE", "number": "1"},
                {"count": 3, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 1, "name": "Basic Fighting Energy", "set": "SVE", "number": "6"},
                {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                {"count": 1, "name": "Jet Energy", "set": "PAL", "number": "190"},       
            ]
        }
    },
    {
        "firstName": "Rj Squires",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Henry Thompson",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/arceus.png',
        sprite2: '../../../../assets/sprites/giratina-origin.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Giratina VSTAR", "set": "LOR", "number": "131"},
                {"count": 2, "name": "Giratina V", "set": "LOR", "number": "130"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Iron Leaves ex", "set": "TEF", "number": "25"},
                {"count": 1, "name": "Radiant Gardevoir", "set": "LOR", "number": "69"},
                {"count": 1, "name": "Skwovet", "set": "SVI", "number": "151"},
                 ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 3, "name": "Judge", "set": "SVI", "number": "176"},
                {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 2, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 1, "name": "Professor Turo's Scenario", "set": "PAR", "number": "171"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Choice Belt", "set": "PAL", "number": "176"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
                {"count": 2, "name": "Lost City", "set": "LOR", "number": "161"},
            ],
            "energy": [
                {"count": 5, "name": "Basic Grass Energy", "set": "SVE", "number": "1"},
                {"count": 4, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                {"count": 1, "name": "Jet Energy", "set": "PAL", "number": "190"},       
            ]
        }
    },
    {
        "firstName": "Raúl Santander",
        flag: '../../../../assets/flags/spain.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Justin Templer",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Kamil Bidzinski",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Oliver Hanns",
        flag: '../../../../assets/flags/australia.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Luka Levain",
        flag: '../../../../assets/flags/france.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Tristan Tse",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Daniel Magda",
        flag: '../../../../assets/flags/czech-republic.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Zubin Schweitzer",
        flag: '../../../../assets/flags/uk.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Martin Erlandsson",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/arceus.png',
        sprite2: '../../../../assets/sprites/giratina-origin.png',
        list: '../../../../assets/list-icon.png',
        "decklist": {
            "pokemon": [
                {"count": 4, "name": "Arceus VSTAR", "set": "BRS", "number": "123"},
                {"count": 4, "name": "Arceus V", "set": "BRS", "number": "122"},
                {"count": 2, "name": "Giratina VSTAR", "set": "LOR", "number": "131"},
                {"count": 2, "name": "Giratina V", "set": "LOR", "number": "130"},
                {"count": 2, "name": "Bibarel", "set": "BRS", "number": "121"},
                {"count": 2, "name": "Bidoof", "set": "CRZ", "number": "111"},
                {"count": 1, "name": "Iron Leaves ex", "set": "TEF", "number": "25"},
                {"count": 1, "name": "Radiant Gardevoir", "set": "LOR", "number": "69"},
                {"count": 1, "name": "Spiritomb", "set": "PAL", "number": "89"},
                {"count": 1, "name": "Skwovet", "set": "SVI", "number": "151"},
                 ],
            "trainer": [
                {"count": 4, "name": "Iono", "set": "PAF", "number": "80"},
                {"count": 3, "name": "Judge", "set": "SVI", "number": "176"},
                {"count": 3, "name": "Boss’s Orders (Ghetsis)", "set": "PAL", "number": "172"},
                {"count": 2, "name": "Eri", "set": "TEF", "number": "146"},
                {"count": 4, "name": "Ultra Ball", "set": "PAF", "number": "91"},
                {"count": 4, "name": "Nest Ball", "set": "PAF", "number": "84"},
                {"count": 1, "name": "Buddy-Buddy Poffin", "set": "TEF", "number": "144"},
                {"count": 1, "name": "Lost Vacuum", "set": "CRZ", "number": "135"},
                {"count": 1, "name": "Switch", "set": "SVI", "number": "194"},
                {"count": 1, "name": "Counter Catcher", "set": "PAR", "number": "160"},
                {"count": 1, "name": "Maximum Belt", "set": "TEF", "number": "154"},
                {"count": 1, "name": "Lost City", "set": "LOR", "number": "161"},
            ],
            "energy": [
                {"count": 5, "name": "Basic Grass Energy", "set": "SVE", "number": "1"},
                {"count": 4, "name": "Basic Psychic Energy", "set": "SVE", "number": "5"},
                {"count": 4, "name": "Double Turbo Energy", "set": "BRS", "number": "151"},
                {"count": 1, "name": "Jet Energy", "set": "PAL", "number": "190"},       
            ]
        }
    },
]
let juniors = [
    {
        "firstName": "Yohann Cote",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/goodra-hisui.png',
        sprite2: '../../../../assets/sprites/comfey.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Peter Shapkin",
        flag: '../../../../assets/flags/uk.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/lugia.png',
        sprite2: '../../../../assets/sprites/cinccino.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Kingsley C",
        flag: '../../../../assets/flags/australia.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Drake Z",
        flag: '../../../../assets/flags/new-zealand.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Gregory V",
        flag: '../../../../assets/flags/france.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Arissa W",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Reed K",
        flag: '../../../../assets/flags/canada.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Zachary O",
        flag: '../../../../assets/flags/uk.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Sebastian L",
        flag: '../../../../assets/flags/new-zealand.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Pyettra S",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Flavio F",
        flag: '../../../../assets/flags/italy.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Mattia B",
        flag: '../../../../assets/flags/switzerland.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Marcelo R",
        flag: '../../../../assets/flags/brazil.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Henry C",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Zelda Z",
        flag: '../../../../assets/flags/usa.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
    {
        "firstName": "Mateusz S",
        flag: '../../../../assets/flags/poland.png',
        "event": "2024 EUIC",
        sprite1: '../../../../assets/sprites/blank.png',
        sprite2: '../../../../assets/sprites/hyphen.png',
        // list: '../../../../assets/list-icon.png',
        list: '../../../../assets/sprites/blank.png',
        "decklist": {
            "pokemon": [
                { "count": 3, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "trainer": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ],
            "energy": [
                { "count": 4, "name": "ee", "number": "ee", "set": "ee" },
            ]
        }
    },
]